...
- **The football field chart (openpyxl) needed hand-debugging against real Excel rendering.** Formula correctness alone doesn't guarantee chart layout/formatting survives identically - axis title placement and explicit `axPos` settings shifted in ways that weren't obvious from the underlying values. The chart's category-vs-value axis semantics in openpyxl were also a real source of a bug: `x_axis` is always the category axis and `y_axis` is always the value axis, regardless of visual bar orientation - counter to what the visual layout suggests. Getting this chart right took three rounds of real-Excel-screenshot verification.
...
- **Currency mismatches are detected and converted, not silently ignored.** A company can trade in one currency while reporting financials in another (e.g. Sony: USD-traded ADR, JPY-reported; AB InBev: EUR-traded, USD-reported) - dividing one currency's enterprise value by another currency's share count produces a meaningless number with no error or crash (first found on SONY, as a "+25,400% upside" that was a pure unit error, not a valuation finding). `check_currency_mismatch()` in `step2_get_financials.py` detects this by comparing yfinance's `currency` (trading) and `financialCurrency` (reporting) fields; `fx_conversion.py` then converts the financials into the trading currency at the current spot rate (free, via the Frankfurter/ECB API, no key required) before the DCF runs, and the app discloses the conversion and rate used rather than presenting it as if no conversion happened. This trades a small amount of precision (one current rate applied across all historical years, not each year's own historical rate) for a usable result instead of an outright block.

## Tech stack

`Python` · `yfinance` · `pandas` · `numpy` · `openpyxl` · `Anthropic Claude API` · `Streamlit` · `matplotlib`
...
Opens a browser tab with a sidebar for ticker, WACC, terminal growth, and toggles for every optional feature (comps, Monte Carlo, backtest, AI summary). Don't know a company's ticker? Type its name instead (e.g. "Microsoft") - `ticker_lookup.py` resolves it via Yahoo Finance's own search (free, no API key) and lets you pick the right match from a short list; input that already looks like a ticker skips this entirely and runs exactly as before. Click "Run Analysis" to see a rule-based valuation rating, key metrics, a resampled price history chart, three donut charts (capital structure, analyst recommendations, and what share of the DCF's enterprise value comes from the explicit 5-year forecast vs. the terminal value - a standard valuation sanity check), and a color-graded WACC/growth sensitivity table matching the Excel workbook's conditional formatting - then download the full Excel workbook. Also guards against a real data-integrity issue found during testing: some non-US companies (e.g. Sony, AB InBev) trade in one currency but report financials in another, which silently produces a meaningless implied price if not caught - the app detects this, converts the financials into the trading currency at the current spot rate, and discloses exactly what it did rather than either showing a wrong number or blocking the analysis outright.
...
MIT - see [LICENSE](LICENSE). Use, modify, and reuse freely; not investment advice.
