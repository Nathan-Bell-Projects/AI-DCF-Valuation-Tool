...
- **Single currency assumption, now actively guarded against.** Financials are used as reported by yfinance; no currency conversion is applied. This was originally a theoretical limitation, until testing SONY (a USD-traded ADR whose financials are reported in JPY) produced a "+25,400% upside" - a pure unit error, not a valuation finding. `check_currency_mismatch()` in `step2_get_financials.py` now compares yfinance's `currency` (trading) and `financialCurrency` (reporting) fields and blocks the analysis with a clear explanation in `app.py` when they differ, rather than silently producing a meaningless number.
